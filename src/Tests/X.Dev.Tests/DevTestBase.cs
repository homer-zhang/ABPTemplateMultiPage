...
namespace X.Dev.Tests
{
    public abstract class DevTestBase : AbpIntegratedTestBase<DevTestModule>
    {
        ...
        protected void UsingDbContext(Action<DevDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<DevDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<DevDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<DevDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<DevDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<DevDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<DevDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<DevDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<DevDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<DevDbContext, Task<T>> func)
        {
            T result;
